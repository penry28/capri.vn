...
<head>
    ...
    <title>Admin | Update bài viết</title>
</head>
                                                                        ...
                                                                        <div class="wrap-banner-form d-flex" style="position: relative;">
                                                                            <div class="banner-img-demo">
                                                                                <span class="thumbNail" style="width: 328px;height: 328px;">
                                                                                    <img id="img-thumbNail-inner" data-exist-img="false" src="<?php if(!empty(set_value("banner_img_thumbNail_url"))) { echo set_value("banner_img_thumbNail_url"); } else { if(!empty($catalogueItem['catalogue_banner'])) { echo $catalogueItem['catalogue_banner']; } else { echo "./public/images/logo/img_prod.png"; } } ?>" alt="<?php if(!empty(set_value("title_catalogue"))) { echo set_value("title_catalogue"); } else { if(!empty($catalogueItem['catalogue_title'])) { echo $catalogueItem['catalogue_title']; } else { echo "Banner catalogue"; } } ?>">
                                                                                    <input id="img-thumbNail-inner-url" name="banner_img_thumbNail_url" type="hidden" value="<?php if(!empty(set_value("banner_img_thumbNail_url"))) { echo set_value("banner_img_thumbNail_url"); } else { if(!empty($catalogueItem['catalogue_banner'])) { echo $catalogueItem['catalogue_banner']; } else { echo null; } } ?>">
                                                                                </span>
                                                                            </div>
                                                                    ...
                                                                    <div class="form-info-right form-info-item-wrap">
                                                                        ...
                                                                        <div class="info-wrap-form">
                                                                            <div class="form-group">
                                                                                ...
                                                                                <input type="text" name="title_catalogue" value="<?php if(!empty(set_value("title_catalogue"))) { echo set_value("title_catalogue"); } else { if(!empty($catalogueItem['catalogue_title'])) { echo $catalogueItem['catalogue_title']; } else { echo null; } } ?>" id="title-catalogue" class="form-control" placeholder="Tiêu đề catalogue" autocomplete="off" spellcheck="false">
                                                                                <?php echo form_error("title_catalogue"); ?>
                                                                            </div>
                                                                            <div class="form-group">
                                                                                <label for="link-download" class="primary-title">Link download catalogue</label>
                                                                                <input type="text" name="link_download" value="<?php if(!empty(set_value("link_download"))) { echo set_value("link_download"); } else { if(!empty($catalogueItem['catalogue_link'])) { echo $catalogueItem['catalogue_link']; } else { echo null; } } ?>" id="link-download" class="form-control" placeholder="Link download catalogue" autocomplete="off" spellcheck="false">
                                                                                <?php echo form_error("link_download"); ?>
                                                                            </div>
                                                                            <div class="form-group">
                                                                                <label for="year-created" class="primary-title">Năm xuất bản catalogue</label>
                                                                                <input type="text" name="year_created" value="<?php if(!empty(set_value("year_created"))) { echo set_value("year_created"); } else { if(!empty($catalogueItem['catalogue_year'])) { echo $catalogueItem['catalogue_year']; } else { echo null; } } ?>" id="year-created" class="form-control" placeholder="Năm xuất bản catalogue" autocomplete="off" spellcheck="false">
                                                                                <?php echo form_error("year_created"); ?>
                                                                            </div>
                                                                            <div class="form-group">
                                                                                <button type="submit" name="update_new_catalogue" class="add-new-cate">Update catalogue</button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
